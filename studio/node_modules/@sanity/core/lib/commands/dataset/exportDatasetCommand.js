  ...
  action: (() => {
    var _ref = _asyncToGenerator(function* (args, context) {
      ...
      const dataset = _args$argsWithoutOpti[0],
            destination = _args$argsWithoutOpti[1];

      const signature = 'sanity dataset export [dataset] [destination]';

      if (!dataset) {
        throw new Error(`Dataset must be specified ("${signature}")`);
      }

      if (!destination) {
        throw new Error(`Destination filename must be specified. Use "-" to print to stdout. ("${signature}")`);
      }
      ...
      const outputPath = yield getOutputPath(destination, dataset
      ...
      (0, _streamDataset2.default)(client, dataset).pipe((0, _split2.default)()).pipe(_skipSystemDocuments2.default).pipe(outputPath ? _fsExtra2.default.createWriteStream(outputPath) : process.stdout).on('error', function (err) {
